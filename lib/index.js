(function(factory, window) {
  ...
  function drawLine(map) {
    ...
    for (var i = 0; i < markerList.length; i++) {
      ...
      var markerPosition = L.DomUtil.getPosition(markerDom);
      var label = marker.getTooltip();

      var labelDom = label._container;
      var labelPosition = L.DomUtil.getPosition(labelDom);
  ...
  function computePositionStep(t) {
    ...
    for (i = 0; i < markerList.length; i++) {
      ...
      v.disp = L.point(0, 0);
      v_pos = L.DomUtil.getPosition(v.getTooltip()._container);
      ...
      for (var j = 0; j < markerList.length; j++) {
        var u = markerList[j];
        if (i !== j) {
          var u_pos = L.DomUtil.getPosition(u.getTooltip()._container);
    ...
    for (i = 0; i < markerList.length; i++) {
      v = markerList[i];
      v_pos = L.DomUtil.getPosition(v.getTooltip()._container);
      dpos = v_pos.subtract(L.DomUtil.getPosition(v._icon));
    ...
    for (i = 0; i < markerList.length; i++) {
      ...
      var p = L.DomUtil.getPosition(markerList[i].getTooltip()._container);
      var d = normalize(disp).scaleBy(
        L.point(Math.min(Math.abs(disp.x), t), Math.min(Math.abs(disp.y), t))
      );
      p = p.add(d);
      p = L.point(Math.ceil(p.x), Math.ceil(p.y));
      L.DomUtil.setPosition(markerList[i].getTooltip()._container, p);
    }
  }
  ...
  function setEdgePosition() {
    ...
    for (let i = 0; i < markerList.length; i++) {
      var tooltip = L.DomUtil.getPosition(markerList[i].getTooltip()._container);
      var marker = L.DomUtil.getPosition(markerList[i]._icon);
      var width = markerList[i].getTooltip()._container.offsetWidth;
      var height = markerList[i].getTooltip()._container.offsetHeight;
      ...
      L.DomUtil.setPosition(markerList[i].getTooltip()._container, tooltip);
    }
  }
